...
- [ ] **1.1 — Turn this folder into a git repo & commit the planning docs.** Visible: `git log --oneline` shows one commit; `git status` clean.
- [ ] **1.2 — Create & activate a Python virtual environment.** Visible: shell prompt shows `(.venv)`; `which python` points inside `.venv`.
- [ ] **1.3 — Install Flask & pin it in `requirements.txt`.** Visible: `requirements.txt` exists with pinned versions; `pip list` shows Flask; committed.
