...
| Pure functions (no side effects, testable in isolation) | practicing | 2026-08-08 | 2026-08-08 | 2026-08-08 task 6.5: wrote `next_due_at(rating, now)` with no clock/DB inside; quiz — `now` as argument so tests can pass a fixed time (correct). |
| Reading an error message / stack trace | practicing | 2026-08-02 | 2026-08-08 | Prior Jinja. 2026-08-08 task 6.6: SyntaxError on `datetime.now()isoformat()` (missing `.`) and JS `elif` vs `else if` — fixed from compiler/editor messages. |
...
| URL routes mapped to Python functions | practicing | 2026-08-01 | 2026-08-09 | Prior routes. 2026-08-09 task 7.1: added `@app.route("/coverage")` + `coverage()` — fourth page, plain path (no path/query params yet). |
...
| Query parameters (`?spec_point=<id>`) | practicing | 2026-08-08 | 2026-08-09 | Prior. 2026-08-09 task 7.6: `url_for('study', spec_point=…)` from coverage → `/study?spec_point=…`. |
| HTTP status codes (200, 302, 404, 500) | practicing | 2026-08-01 | 2026-08-08 | Prior codes. 2026-08-08 task 6.1: distinguished 404 (server said missing path) from no HTTP response at all when Flask isn’t running. |
