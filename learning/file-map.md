...
| `app.py` | known | Flask entry point. Routes for tree/cards/study/coverage; `/auth-check` env probe (9.1); `@app.before_request` HTTP basic auth against `APP_PASSWORD` (9.2). |
| `db.py` | known | DB access layer. `get_connection()` opens SQLite with `Row` factory + FK pragma. `DB_PATH` is absolute via `os.path.dirname(__file__)` so PA’s web worker doesn’t open an empty DB in the wrong cwd (task 9.5). `load_spec_tree()` rebuilds nested nodes. |
...
| &nbsp;&nbsp;`templates/base.html` | known | Shared shell: title block, viewport meta (task 9.3), CSS `<link>`, empty `content` block, and `<script src="{{ url_for('static', filename='tree.js') }}">` after content so the DOM exists before JS runs. |
...
| &nbsp;&nbsp;`static/style.css` | known | Layout, tree, study cards; `.no-cards` highlight; `@media (max-width: 600px)` mobile tweaks (task 9.3). |
| &nbsp;&nbsp;`static/tree.js` | known | Click handler: toggles `is-open` on the child `<ul>` *and* the button (so CSS can flip the caret). |
| &nbsp;&nbsp;`static/study.js` | known | Study keyboard: Space flips; keys 1–5 rate. Phone friction (no Space) noted end of 9.6 — tap-to-flip pending. |
...
| `Dockerfile` | known | Recipe to package the app: `python:3.12-slim` → `WORKDIR` → copy/`pip install` from `requirements.txt` → copy code → `flask run --host=0.0.0.0 --port=5000`. Authored 2026-08-13 in task 9.4. Build: `docker build -t flashie .` |
| `.dockerignore` | known | Like `.gitignore` for `docker build` — keeps `.venv/`, `__pycache__/`, `.git/`, `.pytest_cache/` out of the image. Authored 2026-08-13 in task 9.4. |
...
- PythonAnywhere WSGI file lives on PA under `/var/www/...` (not in this repo) — edited on PA in 9.5; revisit only when changing deploy config.
- `.env` — optional local secrets file. Never on GitHub (`.gitignore`).
- `fly.toml` — parked; hosting moved to PythonAnywhere 2026-08-13.
